node {
    ...
    withCredentials([sshUserPrivateKey(credentialsId: '2b399dca-949a-46ca-945e-707e04291394', keyFileVariable: 'identity', passphraseVariable: '', usernameVariable: 'userName')]) {
        ...
        stage('Remote SSH') {
            try {
                sshCommand remote: remote, command: "cd /opt/yeargin-hubot/ && git pull && docker pull yeargin/yeargin-hubot:build-${env.BUILD_NUMBER} && docker-compose build && docker-compose restart"
